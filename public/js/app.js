...
const ICON = {
  ...
  video: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 8-6 4 6 4V8Z"/><rect x="2" y="6" width="14" height="12" rx="2" ry="2"/></svg>',
      ...
      <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
          ...
          <button class="nav-item ${state.currentPage === 'generate' ? 'active' : ''}" data-page="generate">
            <span class="icon">${ICON.palette}</span> 创作生成
          </button>
  ...
  container.innerHTML = `
    ...
    <div class="generate-container">
      <div class="generate-form">
        <div class="mode-switch" style="display:flex;gap:8px;margin-bottom:16px">
          <button class="type-switch-btn ${!isVideo ? 'active' : ''}" data-mode="image">${ICON.image} 图片生成</button>
          <button class="type-switch-btn ${isVideo ? 'active' : ''}" data-mode="video">${ICON.video} 视频生成</button>
        </div>

        <div class="type-switch">
          ${isVideo ? `
            <button class="type-switch-btn ${state.generateType === 'text-to-video' ? 'active' : ''}" data-type="text-to-video">${ICON.sparkles} 文生视频</button>
            <button class="type-switch-btn ${state.generateType === 'image-to-video' ? 'active' : ''}" data-type="image-to-video">${ICON.film} 图生视频</button>
          ` : `
      ...
      <div class="generate-preview">
        ...
        <div class="result-area ${state.generating ? '' : 'empty'}" id="result-area">
          ...
          ` : `
            <div class="result-icon">${isVideo ? ICON.video : ICON.image}</div>
...
function showResult(task) {
  ...
  if (outputs.length === 0) {
    resultArea.innerHTML = `<div class="empty-state"><div class="empty-icon">${ICON.x}</div><p>未获取到结果</p></div>`;
    return;
  }
      ...
      ${outputs.map(url => {
        if (isVideoTask || url.endsWith('.mp4') || url.includes('video')) {
          return `<div class="result-video-item">
            <video src="${url}" controls preload="metadata" style="width:100%;border-radius:12px"></video>
            <a class="download-btn" href="${url}" download target="_blank">${ICON.download} 下载视频</a>
          </div>`;
        }
  ...
  container.innerHTML = `
    ...
    <div class="task-filters">
      ...
      <select id="filter-type">
        ...
        <option value="text-to-video" ${state.taskFilterType === 'text-to-video' ? 'selected' : ''}>文生视频</option>
        <option value="image-to-video" ${state.taskFilterType === 'image-to-video' ? 'selected' : ''}>图生视频</option>
      </select>
        ...
        <table class="task-table">
          ...
          <tbody>
            ${state.tasks.map(t => {
              ...
              <tr>
                <td>#${t.id}</td>
                <td>${thumb ? (videoType || thumb.endsWith('.mp4') ? `<video src="${thumb}" class="task-thumb" muted preload="metadata" onmouseover="this.play()" onmouseout="this.pause()"></video>` : `<img src="${thumb}" class="task-thumb" onclick="viewImage('${thumb}')">`) : '<span style="color:var(--dark-light);font-size:12px">无</span>'}</td>
            ...
            <div class="task-output-images">
              ${outputs.map(url => {
                if (videoType || url.endsWith('.mp4') || url.includes('video')) {
                  return `<video src="${url}" controls preload="metadata" style="width:100%;border-radius:12px;max-height:400px"></video>`;
                }
  ...
  container.innerHTML = `
    ...
    <div class="task-filters">
      ...
      <select id="admin-filter-type">
        ...
        <option value="text-to-video" ${state.allTaskFilterType === 'text-to-video' ? 'selected' : ''}>文生视频</option>
        <option value="image-to-video" ${state.allTaskFilterType === 'image-to-video' ? 'selected' : ''}>图生视频</option>
      </select>
